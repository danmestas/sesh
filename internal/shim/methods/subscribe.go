...
func (d *Dispatcher) SubscribeToTask(w http.ResponseWriter, r *http.Request, reqID json.RawMessage, params json.RawMessage) {
	if d.deps.JS == nil {
		jsonrpc.WriteError(w, reqID, jsonrpc.ErrInternal.WithData(map[string]string{"reason": "JetStream v2 not configured"}))
		return
	}
	var p subscribeParams
	if len(params) == 0 {
		jsonrpc.WriteError(w, reqID, jsonrpc.ErrInvalidParams.WithData(map[string]string{"err": "params is required"}))
		return
	}
	if err := json.Unmarshal(params, &p); err != nil {
		jsonrpc.WriteError(w, reqID, jsonrpc.ErrInvalidParams.WithData(map[string]string{"err": err.Error()}))
		return
	}
	if p.ID == "" {
		jsonrpc.WriteError(w, reqID, jsonrpc.ErrInvalidParams.WithData(map[string]string{"err": "id is required"}))
		return
	}
	...
	if _, err := tasksKV.Get(r.Context(), p.ID); err != nil {
		if errors.Is(err, jetstream.ErrKeyNotFound) {
			jsonrpc.WriteError(w, reqID, jsonrpc.ErrTaskNotFound)
			return
		}
		d.deps.Log.Error("SubscribeToTask: read task", "task_id", p.ID, "err", err)
		jsonrpc.WriteError(w, reqID, jsonrpc.ErrInternal)
		return
	}
